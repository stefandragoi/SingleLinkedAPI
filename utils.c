...
void add_node(pthread_mutex_t* mutex, int data) {
...
void delete_node(pthread_mutex_t* mutex, int data) {
...
void print_list(pthread_mutex_t* mutex) {
	struct node* current_node;
    pthread_mutex_lock(mutex);
	current_node = head;
...
void sort_list(pthread_mutex_t* mutex) {
	int count, i;
	...
	for (i = 0; i < count - 1; i++) {
		...
		while(NULL != node_j && NULL != node_j->next) {
			if (node_j->value > node_j->next->value) {
				if (prev_j == NULL) // swapping list's head, no previous element
					head = node_j->next;
				else
			...
			// advance forward through the list
			prev_j = node_j;
			node_j = node_j->next;
		}
	}
...
void flush_list(pthread_mutex_t* mutex) {
	struct node* current_node;
	struct node* aux_node;
